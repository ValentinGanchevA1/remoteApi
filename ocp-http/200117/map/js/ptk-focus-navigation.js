...
if (typeof PTK.FocusNavigation == "undefined") {
    ...
    PTK.FocusNavigation = Class.create({
        ...
        onSearchClick: function(d) {
            ...
            var b = this;
            var c;
            $$(".opl-navigation__mobile-visible").each(function(f) {
                c = f.select(b.options.searchTriggerSelector);
                if (c.length) {
                    throw $break
                }
            });
            if (c.length) {
                if (c[0].hasClassName("active")) {} else {
                    c[0].addClassName("active");
                    $$(this.searchTrigger).each(function(f) {
                        f.setStyle({
            ...
            if (this.searchContent.hasClassName("is-active")) {
                ...
                if (this.activeMenuElement) {
                    this.hideMenu(d, function() {
                        b.searchContent.addClassName("is-active");
                        b.isAnimated = false
                    })
                } else {
                    b.searchContent.addClassName("is-active")
        ...
        closeSearch: function(d) {
            if (this.searchContent.hasClassName("is-active")) {
                if ((d && d.target.nodeName.toLowerCase() == "a") && (d.target.hasClassName("opl-navigation__search-trigger"))) {
                    var f = $("searchPhrase");
                    f.setValue("");
                    var a = $("search-result");
                    a.hide()
                    ...
                    $$(".opl-navigation__mobile-visible").each(function(e) {
                        c = e.select(b.options.searchTriggerSelector);
                        if (c.length) {
                            throw $break
                        }
                    });
                    ...
                    a.hide();
                    if (c.length) {
                        if (c[0].hasClassName("active")) {
                            c[0].removeClassName("active")
                        } else {
                            c[0].addClassName("active")
                        }
                    }
