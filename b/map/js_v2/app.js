...
define("underscoreDeepExtend", function() {});
...
define('adapter/enumerable', ['require', 'jq', 'underscore', 'underscoreDeepExtend'], function(require, jquery, underscore, deep) {
...
define('adapter/dom', ['require', 'jq', 'adapter/enumerable'], function(require, $, enumerable) {
    //    var $ = require('jquery');
    ...
    var public = {
		...
		*/
        ...
        append: function(el, content, position) {
            ...
            if ($content && $content.length && $($content).filter(function() {
        ...
        /**
         * Metoda zastępuje tekst w podanym elemencie
         *
         * @method text
         * @param el
         * @param text
        ...
        remove: function(el, fade) {
            ...
            //sprawdzamy czy element nie jest przypadkim undefined lub null
        ...
        /**
         * Zwraca obecną pozycję pionowego scrolla dla pierwszego z dopasowanych elementów lub ustawia tą wartość
         * @method scrollTop
         * @param value        (opcjonalny) o ile px przeskrolować stronę
         * @return        wartość scrolla
         */
        ...
        /**
         * Zwraca obecną pozycję poziomego scrolla dla pierwszego z dopasowanych elementów lub ustawia tą wartość
         * @method scrollLeft
         * @param value        (opcjonalny) o ile px przeskrolować stronę
         * @return        wartość scrolla
         */
        ...
        /**
         * Metoda do wywolania change na elemencie typu radio lub checkbox
         */
        ...
        /**
         * Dodaje lub usuwa podaną klasę na elemencie
         * @method toggleClass
         * @param el                    element lub selektor elementu
         * @param {String} className    nazwa klasy do usunięcia
         */
        ...
        /**
         * Zwraca true/false w zależności czy element contained zawiera się w container
        ...
        ajax: function(url, type, data, context, dataType, successHandler, errorHandler, options) {
            ...
            if (!url) {
                if (window.console) console.warn('@Adapter: dom.ajax: url jest wymagany'); // @DEV
                return;
            }
        ...
        getScript: function(url, success) {
            if (!url) {
                if (window.console) console.warn('@Adapter: dom.getScript: url jest wymagany'); // @DEV
                return;
            }
...
/*
 * v1.5.2
 * https://github.com/mroderick/PubSubJS/tree/v1.5.2
 * */
...
define('events/event', [], function() {
    ...
    ns.events = {
        ...
        expanderEventDone: 'expander.event.done',
        expanderEventExpand: 'expander.event.expand',
        expanderEventCollapse: 'expander.event.collapse',
        ...
        invoicesShowAuth: 'invoices.show.auth',
        ...
        validationEventValid: 'validation.event.valid',
        validationEventInValid: 'validation.form.notvalid',

        landLineCheckboxChange: 'landline.checkbox.change'
...
//require(['adapter/dom', 'pubsub', 'adapter/enumerable', 'events/event', 'common/utils','lib/fastclick'], function (dom, pubSub, enumerable, evtNames, utils, fastclick) {
require(['adapter/dom', 'pubsub', 'adapter/enumerable', 'events/event', 'common/utils'], function(dom, pubSub, enumerable, evtNames, utils) {
    var exportedModules = namespace('P.page.modules');
    ...
    var Core = {
        ...
        start: function(moduleId, plugin, subname, params) {
            ...
            /* Obsluga pluginow dodawanych w artykulach, z zewnetrznych lokalizacji
             * Sciezka musi byc pelna, natomiast nazwa pluginu powinna nie posiadac ani sciezki ani rozszerzenia .js 
             */

            // zakładamy, że jeżeli mamy ładować plugin JS z zewnętrznej lokalizacji (nie z js/plugins/) to data-js-m musi zawierać jedno z http/s/.js
            var regex = new RegExp('^(http[s]?:/)?(/[0-9A-Za-z-\\.@:%_\+~#=]+)+([.]js)$', "i"), // regexp sprawdzający czy string urlem do JSa (zaczyna się od 'http://', 'https://' lub '/' i kończy na '.js'; pomiędzy '/', a '.js' coś musi być
                isExternalPlugin = regex.test(plugin),
                pluginPath = isExternalPlugin ? plugin : config.pluginsPath + plugin;

            if (isExternalPlugin) {

                /* dla pluginow zewnetrznych dzielimy sciezke po slash-u, 
                 * bierzemy sama nazwe pliku i usuwamy z niej rozszerzenie .js
                 */
                var tmpPlug = plugin.split('/');

                plugin = tmpPlug[tmpPlug.length - 1].split('.js')[0];

            }
            /*Obsluga pluginow dodawanych w artykulach*/
            ...
            p = require([pluginPath], function(p) {
                ...
                self.pagePluginsEvents[pluginIdentifier][evtNames.events.pluginInit] = pubSub.subscribe(pluginEventPrefix + "." + evtNames.events.pluginInit, self.onPluginInit.bind(self));
                self.pagePluginsEvents[pluginIdentifier][evtNames.events.pluginInitError] = pubSub.subscribe(pluginEventPrefix + "." + evtNames.events.pluginInitError, self.onPluginInitError.bind(self));
                self.pagePluginsEvents[pluginIdentifier][evtNames.events.pluginStop] = pubSub.subscribe(pluginEventPrefix + "." + evtNames.events.pluginStop, self.onPluginStop.bind(self));
                self.pagePluginsEvents[pluginIdentifier][evtNames.events.pluginAsk] = pubSub.subscribe(pluginEventPrefix + "." + evtNames.events.pluginAsk, self.pluginAskHandler.bind(self));
                self.pagePluginsEvents[pluginIdentifier][evtNames.events.pluginEvent] = pubSub.subscribe(pluginEventPrefix, self.pluginEventHandler.bind(self));
        ...
        /**
         * Inicjalizuje dodatkowe biblioteki
         * @method additionalLibs
         */
        ...
        startOne: function(plug) {
            var modId = plug.modId,
                ...
                params = plug.params || {},
                el = plug.view || modId,
                pluginId = this.buildPluginId(modId, plugin, subname);


            if (eventNs) enumerable.extend(params, {
                'eventNs': eventNs
            });
            ...
            if (params.dynamic === true && params.event) {
                ...
                if (onlyDevice === 'all' || (onlyDevice === 'touch' && utils.device.isTouch) || (onlyDevice === 'non-touch' && !utils.device.isTouch)) {
                    ...
                    this.onResizeResDepend[pluginId] = onResizeResDepend.bind(this);

                    dom.on(window, 'resize', null, null, this.onResizeResDepend[pluginId]);
        ...
        resDependInit: function(modId, plugin, subname, params, minRes, maxRes, initMediaQuery) {
            ...
            if (doInitialization) {
                ...
                this.stop(this.buildPluginId(modId, plugin, subname), true);
            }
        },
         ...
         */
        eventInit: function(modId, plugin, subname, params, el, event) {
            if (params.eventPreventDefault) {
                event.preventDefault();
            }
         ...
         */
        stop: function(pluginId, resDepend) {
            ...
            if (resDepend !== true && this.onResizeResDepend[pluginId]) {

                dom.off(window, 'resize', null, null, this.onResizeResDepend[pluginId]);

                delete this.onResizeResDepend[pluginId];
        ...
        getPluginsFromContext: function(context) {
            ...
            // pobiera drugą listę pluginów z tagu <script> umieszczonego w htmlu
            if (window.plugins2) {
                plugs2 = enumerable.clone(window.plugins2);
                plugs = enumerable.defaults(plugs, plugs2);
            }
        ...
        onPluginInit: function(event, data) {
            ...
            pubSub.unsubscribe(self.pagePluginsEvents[data.eventSource][evtNames.events.pluginInit]);
            pubSub.unsubscribe(self.pagePluginsEvents[data.eventSource][evtNames.events.pluginInitError]);
        },
        ...
        onPluginStop: function(event, data) {
            ...
            //pubSub.unsubscribe(self.pagePluginsEvents[data.eventSource][evtNames.events.pluginStop]);
         ...
         * jeśli są takie - rzuca do nich event (evtNames.events.pluginActionCall) z informacją o danej akcji
         * uruchamia nasłuchiwanie na odpowiedzi (evtNames.events.pluginActionAnswer) od pluginów
        ...
        pluginAskHandler: function(event, data) {
            ...
            if (!interestedPlugins.length) {
                ...
                pubSub.publish(pluginEventPrefix + evtNames.events.pluginAnswer, answerData);
                ...
                enumerable.each(interestedPlugins, function(intPlugin, index) {
                    ...
                    var token = pubSub.subscribe(prefix + evtNames.events.pluginActionAnswer, self.onPluginActionAnswer.bind(self));
                    ...
                    this.log('dispatch action ' + prefix + evtNames.events.pluginActionCall, evData);
                    pubSub.publish(prefix + evtNames.events.pluginActionCall, evData);
                }, this);
        ...
        pluginEventHandler: function(event, data) {
            ...
            if (eventName != evtNames.events.pluginInit &&
                eventName != evtNames.events.pluginInitError &&
                eventName != evtNames.events.pluginStop &&
                eventName != evtNames.events.pluginAsk &&
                eventName != evtNames.events.pluginInnerAjaxed) {
                var eventNsPlugins = this.getModulePluginsByNamespace(eventNs);
        ...
        onPluginActionAnswer: function(event, data) {
            ...
            if (this.actionThreads[data.actionId].answered == this.actionThreads[data.actionId].asked) {
                ...
                pubSub.publish(this.actionThreads[data.actionId].sourcePrefix + evtNames.events.pluginAnswer, answerData);
            }
        },
         ...
         * @param {string|evtNames.events.eventName} message
         * @param {object} data
         */
     ...
     */
    pubSub.subscribe(evtNames.events.domUpdate, function(e, data) {
        Core.startAll(data.update);
        ...
        enumerable.each(parentsPlugins, function(plugin) {
            ...
            this.log('dispatch action ' + prefix + evtNames.events.pluginInnerAjaxed, data.update.id);
            pubSub.publish(prefix + evtNames.events.pluginInnerAjaxed, data.update.id);
        }, Core);
...
(function(context, OBJECT, NUMBER, LENGTH, toString, version, undefined, oldClass, jsface) {
    ...
    function extend(object, subject, ignoredKeys) {
        if (isArray(subject)) {
            ...
            if (isMap(subject)) {
                for (key in subject) copier(key, subject[key], ignoredKeys, object, iClass, oPrototype);
            }
...
define('adapter/jsClass', ['require', 'jsface'], function(require) {
...
define('common/basePlugin', ['require', 'adapter/dom', 'adapter/enumerable', 'adapter/jsClass', 'pubsub', 'common/utils'], function(require) {
    ...
    var BasePlugin = jsClass.Class({
        ...
        constructor: function(modId, name, subname, options) {
            ...
            //dodanie stopowanie pluginu na zamkniecie popupa
            pubSub.subscribe(ns.events.ajaxPopupClose, this.handleStopInPopup.bind(this));
        ...
        handleStopInPopup: function(e, data) {
            var innerPopupPlugin = dom.find("#" + this.modId, data.popup);
            //if(innerPopupPlugin.length) this.stop(); //chwilowe wycomentowanie, bo to powoduje, ze popup z danymi nie dzialaja poprawnie
        },
        ...
        /**
         * Funkcja zwraca event dla checkboxow w zaleznosci od przegladarki
         */
        getCheckboxEvent: function() {
            var event = 'change';
            if (!utils.browser.features.supportsSelector(':not(:checked)')) {
                event = ns.events.customCheckboxChange;
            }
...
define('common/scroll-to', ['require', 'adapter/dom', 'adapter/jsClass', 'jq', 'adapter/enumerable'], function(require) {
    var dom = require('adapter/dom'),
        jsClass = require('adapter/jsClass'),
        jquery = require('jq'),
        enumerable = require('adapter/enumerable');
    ...
    var ScrollTo = jsClass.Class({
         ...
         */
        constructor: function(elementId, options, callback) {
            ...
            if (this.options.scrollIfVisible || scrollTop >= 0) {
                jquery('body,html').delay(this.options.delay).animate({
                    scrollTop: scrollTop
                }, {
        ...
        findScrollTop: function() {
            ...
            if (pageScrollTop + headerPositionFix <= scrollElementTop &&
                pageScrollTop + windowHeight - scrollElementHeight >= scrollElementTop) {
                return -1;
            }

            return scrollElementTop - dom.outerHeight(header) - elementPositionFix;
        },

        onScrollFinished: function() {
            if (this.options.setFocus) {
                if (this.options.focusedElementId) {
                    dom.trigger(dom.find('#' + this.options.focusedElementId), 'focus');
                } else {
                    dom.trigger(this.scrollElement, 'focus');
                }
            }
...
define('common/custom-checkbox', ['require', 'adapter/dom', 'pubsub', 'adapter/jsClass', 'adapter/enumerable'], function(require) {
    var dom = require('adapter/dom'),
        pubSub = require('pubsub'),
        jsClass = require('adapter/jsClass'),
        enumerable = require('adapter/enumerable');
    ...
    var CustomCheckbox = jsClass.Class({
        ...
        createCheckboxHtml: function() {
            var t = this,
                o = this.opts,
                suffix = '-' + this.classNs + '-checkbox',
                classes = this.classNs + '-custom-input ' + this.classNs + '-' + t.inputType + ' ' + (dom.getClass(t.input) || ''),
                checkboxHtml = '<span id="' + this.input.id + suffix + '" class="' + classes + '"><span class="' + this.classNs + '-checkbox-inner"><span></span></span></span>';
            ...
            var label = this.getLabel(this.input.id);
        ...
        initObservers: function() {
            ...
            dom.on(this.customInput, 'click', this.onCustomInputClick.bind(this));
        ...
        onInputChange: function(e) {
            (this.input.checked) ? dom.addClass(this.customInput, this.classNs + '-custom-input-checked'): dom.removeClass(this.customInput, this.classNs + '-custom-input-checked');
            this.changeState(this.input.checked, this.input.disabled, true);

            //usuwanie klasy focused (powoduje brak podswietlenia podczas klikania w label), rozwiązanie tymczasowe
            var label = this.getLabel(this.input.id);
            //SUN-3821, IE najpierw rzuca change potem focus, dlatego odwracamy kolejnosc wykonania za pomoca setTimeout
            setTimeout(function() {
                dom.removeClass(label, this.classNs + '-custom-input-l-focused');
                dom.removeClass(this.customInput, this.classNs + '-custom-input-focused');
            }.bind(this), 0);
        ...
        onInputFocus: function(e) {
            var label = this.getLabel(this.input.id);
            dom.addClass(label, this.classNs + '-custom-input-l-focused');
            dom.addClass(this.customInput, this.classNs + '-custom-input-focused');
        },
        ...
        onCustomInputClick: function(e) {
            ...
            //jezeli radio to zaznaczaj
            if (this.inputType === 'radio') {


                var notTrigger = false;
                //jeśli był zaznaczony wcześniej
                if (this.input.checked) {

                    notTrigger = true;
                }
                this.changeState(true, this.input.disabled, null, notTrigger);
            } else {
                //jezeli checkbox to odwracaj zaznaczenie
                this.changeState(!this.input.checked, this.input.disabled);
            }
            return false;
        },
        ...
        changeState: function(check, disable, onChange, notTriggerFlag) {
            ...
            (check) ? dom.addClass(this.customInput, this.classNs + '-custom-input-checked'): dom.removeClass(this.customInput, this.classNs + '-custom-input-checked');
            (disable) ? dom.addClass(this.customInput, this.classNs + '-custom-input-disabled'): dom.removeClass(this.customInput, this.classNs + '-custom-input-disabled');

            //jezeli wartosc zmieniona poprzez kilkniecie w label, to nie zmieniaj ponownie, tylko ustaw klasy na customInput
            if (!onChange) {
            ...
            if (this.inputType === 'radio') {
                ...
                enumerable.each(relatedRadios, function(radio) {
                    if (radio.id != this.input.id && check) {
                        radio.checked = false;
                        var customRadio = this.getCustomInputById(dom.identify(radio))[0];
                        if (customRadio) {
                            dom.removeClass(customRadio, this.classNs + '-custom-input-checked');
                            this.changeLabelState(radio);
                            //							if(!notTriggerFlag){
                            //								dom.trigger(radio, ns.events.customCheckboxChange, {checked: radio.checked, disabled: radio.disable, input: radio});
                            //							}
                        }
                    }
                }, this);
            ...
            this.changeLabelState(this.input);

            if (!notTriggerFlag) {
                dom.trigger(this.input, ns.events.customCheckboxChange, {
                    checked: check,
                    disabled: disable,
                    input: this.input
                });
        ...
        getClassRelatedGroup: function(input) {
            var groupIndex = this.radiogroupPrefix.length
            var classes = dom.getAttribute(input, 'class');
            var groupName;
            if (classes) {
                var classArr = classes.split(' ');
                enumerable.each(classArr, function(classItem, i) {
                    if (classItem.indexOf(this.radiogroupPrefix) >= 0) {
                        groupName = classItem.slice(groupIndex);
                    };
                }, this);
            }

            if (groupName) {
                var groupRadios = dom.find('input:radio.' + this.radiogroupPrefix + groupName);
                return groupRadios;
            } else {
                return [];
            }
        ...
        changeLabelState: function(input) {
            var label = this.getLabel(input.id);
            if (!label) return;
            (input.checked) ? dom.addClass(label, this.classNs + '-input-label-checked'): dom.removeClass(label, this.classNs + '-input-label-checked');
            (input.disabled) ? dom.addClass(label, this.classNs + '-input-label-disabled'): dom.removeClass(label, this.classNs + '-input-label-disabled');
        },
        ...
        //wyszukuje custom inputa po id inputa html
        getCustomInputById: function(id) {
            var r = dom.find('#' + id + '-' + this.classNs + '-checkbox');
            return r;
        }
...
/**
 * Skrypt do obsługi graficznych checkboxów, radiobuttonów
 * author: jjagoda
 * Date: 18.10.13
 * Require: basePlugin 
 * @module plugins
...
define('plugins/custom-forms', ['require', 'common/basePlugin', 'adapter/jsClass', 'adapter/enumerable', 'adapter/dom', 'pubsub', 'common/custom-checkbox'], function(require) {
    ...
    var BasePlugin = require('common/basePlugin'),
        ...
        pubSub = require('pubsub'),
        CustomCheckbox = require('common/custom-checkbox');

    var CustomForms = jsClass.Class(BasePlugin, {
        ...
        init: function() {
            ...
            this.initCustomFormsInContext(this.body);

            this.subscribeToDomUpdate();
        },
        ...
        handleDomUpdate: function(e, data) {
            this.initCustomFormsInContext(data.update);
        },

        subscribeToDomUpdate: function() {
            pubSub.subscribe(ns.events.domUpdate, this.handleDomUpdate.bind(this));
        }
